use crate::AppState;
use actix_identity::Identity;
use actix_web::{web::{Data, Query, Redirect, ServiceConfig}, HttpMessage, HttpRequest, HttpResponse, get};
use serde::Deserialize;

pub fn cas_config(cfg: &mut ServiceConfig) {
    cfg.service(login)
    .service(logout);
}

#[derive(Deserialize)]
struct AuthResource {
    ticket: Option<String>,
}

#[get("/login")]
async fn login(req: HttpRequest, data: Data<AppState>, auth: Query<AuthResource>) -> Redirect {
    let auth = auth.into_inner();
    match auth.ticket {
        None => {
            Redirect::to(format!(
                "https://secure6.its.yale.edu/cas/login?service=https://{}/api/cas/login",
                &data.base_url
            ))
        }
        Some(ticket) => {
            let cas_url = format!(
                "https://secure6.its.yale.edu/cas/validate?service=https://{}/api/cas/login&ticket={}",
                &data.base_url, ticket
            );
            let body = reqwest::get(cas_url).await.unwrap().text().await.unwrap();
            match body.lines().nth(1) {
                Some(netid) => {
                    Identity::login(&req.extensions(), netid.to_owned()).unwrap();
                    Redirect::to("/logout")
                }
                None => Redirect::to("/login"),
            }
        }
    }
}

#[get("/logout")]
async fn logout(user: Identity) -> HttpResponse {
    user.logout();
    HttpResponse::Ok().finish()
}